...
class OpenApiFacadeTest {
    ...
    @Test
    fun `test get null entries when there is 0 entries count`() {
        val zeroEntriesApiResponse = ApiResponse(
            count = 0,
            entries = null
        )
    ...
    fun `test get api-tipo-2 entry when there is 2 entries`() {
        ...
        val expectedEntryForTwo = listOf(
            ApiResponse.Entry("api-tipo-2", null, null)
        )
    ...
    @Test
    fun `test throw NotFound when there is 3 entries`() {
